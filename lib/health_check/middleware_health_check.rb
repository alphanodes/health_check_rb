...
module HealthCheck
  class MiddlewareHealthcheck
    ...
    def parse_env(env)
      uri = env['PATH_INFO']
      return unless uri =~ %r{^/#{Regexp.escape HealthCheck.uri}(/([-_0-9a-zA-Z]*))?(\.(\w*))?$}
    ...
    def ip_blocked(env)
      ...
      return if HealthCheck.origin_ip_whitelist.any? { |addr| IPAddr.new(addr).include? request_ipaddr }
    ...
    def not_authenticated(env)
      ...
      if auth.provided? && auth.basic? && Rack::Utils.secure_compare(HealthCheck.basic_auth_username,
                                                                     auth.username) && Rack::Utils.secure_compare(
                                                                       HealthCheck.basic_auth_password, auth.password
                                                                     )
